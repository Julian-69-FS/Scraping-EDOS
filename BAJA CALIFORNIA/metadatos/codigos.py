...
class ReglamentoProcessor:
    ...
    def __init__(self, input_folder: str, output_folder: str):
        ...
        self.contenido_json_path = Path(r"C:\Users\julii\Documents\Practicas\drive\BAJA CALIFORNIA\contenido\codigo-contenido.json")#contenidos de los PDFs
        self.contenido_data = None

        # Ruta del JSON con metadatos adicionales
        self.metadatos_json_path = Path(r"C:\Users\julii\Documents\Practicas\drive\BAJA CALIFORNIA\json_metadatos\metadatos_codigos.json")
        self.metadatos_data = None
    ...
    def load_metadatos_data(self):
        """Carga el JSON con los metadatos adicionales (FECHA PER OFIC, ESTATUS, TOMO, URL)"""
        try:
            if self.metadatos_json_path.exists():
                ...
                logger.info(f"Cargados {len(self.metadatos_data)} metadatos desde {self.metadatos_json_path.name}")
                return True
            else:
                logger.warning(f"No se encontró el archivo de metadatos: {self.metadatos_json_path}")
                return False
        except Exception as e:
            logger.error(f"Error cargando JSON de metadatos: {e}")
            return False
    ...
    def find_matching_metadatos(self, titulo: str) -> Optional[Dict[str, Any]]:
        """
        Busca los metadatos correspondientes al título del documento en metadatos_leyes.json
        Retorna un diccionario con los campos: FECHA PER OFIC, ESTATUS, TOMO, URL
        """
        if not self.metadatos_data or not titulo:
            return None
        ...
        for item in self.metadatos_data:
            # Obtener el nombre del JSON de metadatos - Buscar con "NOMBRE" (mayúsculas o minúsculas)
            nombre_json = item.get('NOMBRE') or item.get('nombre') or item.get('Nombre', '')
            if not nombre_json:
                continue

            nombre_json_normalizado = self.normalize_text(nombre_json)

            # Primero intentar match exacto
            if titulo_normalizado == nombre_json_normalizado:
                logger.info(f"Match exacto de metadatos encontrado: {titulo[:50]}...")
                # Extraer los campos requeridos
                return {
                    'FECHA PER OFIC': item.get('FECHA PER OFIC') or item.get('fecha per ofic') or item.get('Fecha Per Ofic'),
                    'ESTATUS': item.get('ESTATUS') or item.get('estatus') or item.get('Estatus'),
                    'TOMO': item.get('TOMO') or item.get('tomo') or item.get('Tomo'),
                    'URL': item.get('URL') or item.get('url') or item.get('Url')
                }

            # Calcular diferencia de longitud entre títulos
            len_diff = abs(len(titulo_normalizado) - len(nombre_json_normalizado))
            len_ratio = len_diff / max(len(titulo_normalizado), len(nombre_json_normalizado))
            ...
            # Calcular similaridad para encontrar el mejor match
            ratio = SequenceMatcher(None, titulo_normalizado, nombre_json_normalizado).ratio()

            # Aumentar umbral a 92% para mayor precisión
            if ratio > best_ratio and ratio > 0.92:
                # Verificar que no tenga palabras únicas problemáticas
                palabras_titulo = set(titulo_normalizado.split())
                palabras_json = set(nombre_json_normalizado.split())
        ...
        if best_metadatos:
            logger.info(f"Mejor match de metadatos encontrado (similaridad {best_ratio:.2%}): {titulo[:50]}... -> {best_match[:50]}...")
            return best_metadatos

        logger.warning(f"No se encontraron metadatos para: {titulo[:50]}...")
        return None
    ...
    def extract_fecha_publicacion(self, text: str) -> Optional[str]:
        ...
        Busca específicamente el formato del Periódico Oficial del Estado de Baja California:
        "Periódico Oficial del Estado de Baja California, el [día] [día] de [mes] de [año]"
        Ejemplo: "Periódico Oficial del Estado de Baja California, el lunes 14 de agosto de 2017"
        ...
        patrones = [
            # Patrón 1: ESPECÍFICO para Periódico Oficial de Baja California
            # Formato: "Periódico Oficial del Estado de Baja California, el [día_semana] [número] de [mes] de [año]"
            r'Peri[oó]dico\s+Oficial\s+del\s+Estado\s+de\s+Baja\s+California,?\s+el\s+(?:lunes|martes|mi[eé]rcoles|jueves|viernes|s[aá]bado|domingo)\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 2: Versión sin día de la semana
            r'Peri[oó]dico\s+Oficial\s+del\s+Estado\s+de\s+Baja\s+California,?\s+el\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 3: Versión más flexible con saltos de línea
            r'Peri[oó]dico\s+Oficial\s+del\s+Estado\s+de\s*[\r\n\s]*Baja\s+California,?\s+el\s+(?:lunes|martes|mi[eé]rcoles|jueves|viernes|s[aá]bado|domingo)\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',
    ...
    def extract_ultima_reforma(self, text: str) -> Optional[str]:
        ...
        patrones = [
            # Patrón 1: ESPECÍFICO para "ÚLTIMA REFORMA PUBLICADA EN EL PERIÓDICO OFICIAL:"
            r'[UÚ]LTIMA\s+REFORMA\s+PUBLICADA\s+EN\s+EL\s+PERI[OÓ]DICO\s+OFICIAL[:\s]+(\d{1,2}\s+DE\s+\w+\s+DE\s+\d{4})',

            # Patrón 2: Versión más flexible del patrón específico
            r'[UÚ]ltima\s+[Rr]eforma\s+[Pp]ublicada\s+en\s+el\s+[Pp]eri[oó]dico\s+[Oo]ficial[:\s]+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 3: "Última reforma" - MUY FLEXIBLE - formato largo
            r'[UÚ]ltima\s+[Rr]eforma[^\d]{0,50}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 4: "Última actualización" - formato DD/MES/YYYY
            r'[UÚ]ltima\s+[Aa]ctualizaci[oó]n[^\d]{0,30}(\d{1,2}/\w+/\d{4})',

            # Patrón 5: "Última actualización" - formato DD/MM/YYYY
            r'[UÚ]ltima\s+[Aa]ctualizaci[oó]n[^\d]{0,30}(\d{1,2}/\d{1,2}/\d{4})',

            # Patrón 6: "Última reforma" - formato corto DD-MM-YYYY
            r'[UÚ]ltima\s+[Rr]eforma[^\d]{0,30}(\d{1,2}-\d{1,2}-\d{4})',

            # Patrón 7: "Última modificación"
            r'[UÚ]ltima\s+[Mm]odificaci[oó]n[^\d]{0,30}(\d{1,2}[-/]\d{1,2}[-/]\d{4})',
            r'[UÚ]ltima\s+[Mm]odificaci[oó]n[^\d]{0,30}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 8: Solo "reforma" seguido de fecha
            r'[Rr]eforma[^\d]{0,50}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',
            r'[Rr]eforma[^\d]{0,30}(\d{1,2}[-/]\d{1,2}[-/]\d{4})',

            # Patrón 9: Solo "actualización" seguido de fecha
            r'[Aa]ctualizaci[oó]n[^\d]{0,30}(\d{1,2}[-/]\d{1,2}[-/]\d{4})',
            r'[Aa]ctualizaci[oó]n[^\d]{0,30}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 10: "modificación" genérica
            r'[Mm]odificaci[oó]n[^\d]{0,30}(\d{1,2}[-/]\d{1,2}[-/]\d{4})',
            r'[Mm]odificaci[oó]n[^\d]{0,30}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 11: Captura genérica - cualquier fecha en formato DD/MES/YYYY (como 5/ABRIL/2021)
        ...
        for i, patron in enumerate(patrones, 1):
            ...
            if match:
                fecha = match.group(1).strip()
                logger.info(f"Última reforma extraída con patrón #{i}: {fecha}")
                return fecha
    ...
    def process_single_pdf(self, pdf_path: Path) -> Dict[str, Any]:
        ...
        result = {
            ...
            'ordenamiento': 'LEY',  # Tipo de ordenamiento
            'jurisdiccion': 'BAJA CALIFORNIA',  # Jurisdicción
            'fuente_oficial': 'Periódico Oficial',  # Fuente oficial
            'es_escaneado': False,
            'tiene_tablas': False
        }
        ...
        try:
            ...
            return {
                ...
                'ordenamiento': 'LEY',
                'jurisdiccion': 'BAJA CALIFORNIA',
                'fuente_oficial': 'Periódico Oficial',
                'es_escaneado': False,
                'tiene_tablas': False,
                'error': str(e)
            }
    ...
    def process_all_pdfs(self) -> List[Dict[str, Any]]:
        ...
        """
        # Cargar el JSON con los contenidos y metadatos
        logger.info("\n" + "="*60)
        logger.info("Cargando archivos de contenidos y metadatos...")
        ...
        for i, pdf_file in enumerate(pdf_files, 1):
            ...
            try:
                ...
                # Buscar Metadatos correspondientes (FECHA PER OFIC, ESTATUS, TOMO, URL)
                metadatos = self.find_matching_metadatos(result['titulo'])

                # Reorganizar el diccionario con el orden deseado incluyendo 'Contenido' y metadatos
                ordered_result = {
                    ...
                    'FECHA PER OFIC': metadatos.get('FECHA PER OFIC') if metadatos else None,
                    'ESTATUS': metadatos.get('ESTATUS') if metadatos else None,
                    'TOMO': metadatos.get('TOMO') if metadatos else None,
                    'URL': metadatos.get('URL') if metadatos else None,
                ...
                # Intentar buscar Contenido y Metadatos incluso en caso de error
                titulo_temp = self.clean_filename_to_title(pdf_file.stem)
                contenido = self.find_matching_contenido(titulo_temp)
                metadatos = self.find_matching_metadatos(titulo_temp)
...
def main():
    ...
    INPUT_FOLDER = r"C:\Users\julii\Documents\BAJA CALIFORNIA\Codigos" # Carpeta con los PDFs
    OUTPUT_FOLDER = r"C:\Users\julii\Documents\Practicas\drive\BAJA CALIFORNIA\json\Codigos"  # Misma carpeta que el script
    ...
    if len(sys.argv) > 1:
        ...
        if pdf_path.exists():
            ...
            # Cargar Contenidos y Metadatos si están disponibles
            processor.load_contenido_data()
            processor.load_metadatos_data()
            ...
            metadatos = processor.find_matching_metadatos(result['titulo'])
