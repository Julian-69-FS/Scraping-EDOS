...
def corregir_saltos_linea(texto: str) -> str:
    """Corrige saltos de línea artificiales, manteniendo los naturales del PDF

    FILOSOFÍA: Solo unir líneas cuando es OBVIO que es un corte artificial a mitad de frase.
    Mantener los saltos de línea originales cuando la línea parece completa.
    """
    if not texto:
        return texto
    ...
    # Lista para almacenar el texto procesado
    texto_procesado = []
    buffer_linea = []

    for i, linea_actual in enumerate(lineas):
        linea_limpia = linea_actual.strip()

        # Si la línea está vacía, es un separador
        if not linea_limpia:
            if buffer_linea:
                texto_procesado.append(' '.join(buffer_linea))
                buffer_linea = []
            # Mantener la línea vacía como separador
            texto_procesado.append('')
            continue

        # Determinar si la línea ANTERIOR tiene un corte artificial que requiere unión
        debe_unir_con_anterior = False

        if buffer_linea:
            ultima_linea = buffer_linea[-1]

            # CASO 1: Palabra cortada con guión al final (SIEMPRE unir)
            # Ejemplo: "constitu-" + "cional" -> "constitucional"
            if ultima_linea.endswith('-'):
                if re.match(r'^[a-záéíóúñ]', linea_limpia):
                    # Unir sin el guión
                    buffer_linea[-1] = ultima_linea[:-1] + linea_limpia
                    continue

            # CASO 2: Línea anterior termina con conjunción/preposición corta (corte artificial obvio)
            # Ejemplo: "honestidad y" + "objetividad" -> "honestidad y objetividad"
            if re.search(r'\b(y|e|o|u|de|del|al|el|la|los|las|un|una|en|con|sin|por|para|como|sobre|entre|desde|hasta|hacia)\s*$', ultima_linea, re.IGNORECASE):
                # Solo unir si la siguiente línea empieza con minúscula (continuación)
                if re.match(r'^[a-záéíóúñ]', linea_limpia):
                    debe_unir_con_anterior = True

            # CASO 3: Línea anterior NO termina con puntuación Y la siguiente empieza con minúscula
            # Esto indica continuación de la misma frase
            elif not re.search(r'[.!?;:,]\s*$', ultima_linea):
                if re.match(r'^[a-záéíóúñ]', linea_limpia):
                    # Verificar que no sea un inicio de algo estructural
                    if not re.match(r'^(que|como|cuando|donde|aunque|si|porque|para|por)\s+[A-Z]', linea_limpia):
                        debe_unir_con_anterior = True

            # CASO 4: Línea anterior termina con coma (probable continuación)
            elif ultima_linea.endswith(','):
                # Si la siguiente empieza con minúscula, es continuación
                if re.match(r'^[a-záéíóúñ]', linea_limpia):
                    debe_unir_con_anterior = True

        # Si debe unirse con la anterior, agregar al buffer actual
        if debe_unir_con_anterior:
            buffer_linea.append(linea_limpia)
        else:
            # NO unir: la línea anterior estaba completa
            # Guardar la línea anterior (si existe) y empezar nueva
            if buffer_linea:
                texto_procesado.append(' '.join(buffer_linea))
    ...
    # Limpiar múltiples saltos de línea (más de 2) pero mantener hasta 2
    resultado = re.sub(r'\n{3,}', '\n\n', resultado)
...
def eliminar_encabezados_pies_contextual(paginas_texto: List[str], encabezados: List[str], pies: List[str]) -> List[str]:
    """Elimina encabezados y pies de página SOLO en sus posiciones contextuales

    En lugar de hacer un replace() global que eliminaría el texto en cualquier parte,
    esta función elimina las líneas SOLO si aparecen en las primeras/últimas líneas de cada página.
    """
    paginas_limpiadas = []
    ...
    for pagina in paginas_texto:
        ...
        # Crear lista de líneas a mantener
        lineas_mantener = []
        ...
        for i, linea in enumerate(lineas):
            ...
            # Solo verificar si es encabezado si está en las primeras 3 líneas con contenido
            if i < 3:
            ...
            # Solo verificar si es pie si está en las últimas 3 líneas con contenido
            if i >= len(lineas) - 3:
                for pie in pies:
...
def extraer_contenido_pdf(ruta_pdf: str) -> Dict[str, str]:
    ...
    if contenido_completo:
        # Detectar encabezados/pies ANTES de unir, para detectar patrones que se repiten por página
        encabezados, pies = detectar_encabezado_pie(contenido_completo)

        # Aplicar eliminación contextual (solo en posiciones de encabezado/pie de cada página)
        paginas_limpiadas = eliminar_encabezados_pies_contextual(contenido_completo, encabezados, pies)

        # Ahora sí, unir todo el contenido ya limpio
        contenido_final = '\n\n'.join(paginas_limpiadas)

        # Eliminar números de página que puedan haber quedado
        contenido_final = re.sub(r'^\d+\s*$', '', contenido_final, flags=re.MULTILINE)
        contenido_final = re.sub(r'Página\s+\d+', '', contenido_final, flags=re.IGNORECASE)
        contenido_final = re.sub(r'Page\s+\d+', '', contenido_final, flags=re.IGNORECASE)
...
def main():
    ...
    ruta_carpeta = r"C:\Users\julii\Documents\CDMX\marco juridico"

    # Nombre del archivo JSON de salida
    archivo_salida = r"C:\Users\julii\Documents\Practicas\drive\CDMX\json\juridico_docs.json"
