...
class ReglamentoProcessor:
    ...
    def __init__(self, input_folder: str, output_folder: str):
        ...
        # No crear subcarpetas, usar la carpeta especificada directamente
        
        # Inicializar EasyOCR (solo si se necesita)
        self.ocr_reader = None
        
        # Ruta del JSON con URLs
        self.url_json_path = Path(r"C:\Users\julii\Documents\Practicas\drive\CDMX\json\metadatos\m-juridico.json")# metadatos
        self.url_data = None

        # Ruta del JSON con contenidos
        self.contenido_json_path = Path(r"C:\Users\julii\Documents\Practicas\drive\CDMX\json\juridico_docs.json")#contenidos de los PDFs
    ...
    def find_matching_url(self, titulo: str) -> Optional[Dict[str, Any]]:
        """
        Busca la URL y fechas correspondientes al título del documento
        Retorna un diccionario con url, Fecha de Publicación y Fecha de Última Reforma si encuentra match, None si no
        """
        ...
        for item in self.url_data:
            ...
            if titulo_normalizado == titulo_json_normalizado:
                ...
                return {
                    ...
                    'Fecha de Publicación': item.get('FECHA_PUBLICACION'),
                    'Fecha de Última Reforma': item.get('ULTIMA_REFORMA')
                }
        ...
        if best_item:
            ...
            return {
                'url': best_item.get('URL_PDF'),
                'Fecha de Publicación': best_item.get('FECHA_PUBLICACION'),
                'Fecha de Última Reforma': best_item.get('ULTIMA_REFORMA')
            }
    ...
    def process_single_pdf(self, pdf_path: Path) -> Dict[str, Any]:
        ...
        # MODIFICACIÓN: Agregados nuevos campos status, ordenamiento, fuente_oficial y jurisdiccion
        # Estructura simplificada con solo información esencial - Orden reorganizado para legibilidad
        result = {
            'titulo': titulo_base,  # Usar nombre del archivo limpio como título
            'ordenamiento': 'Marco Jurídico',  # MODIFICACIÓN: Campo fijo como "Marco Jurídico"
            'jurisdiccion': 'CIUDAD DE MEXICO',  # NUEVO CAMPO: Jurisdicción fija
            'fuente_oficial': 'Congreso de la Ciudad de México',  # MODIFICACIÓN: Campo fijo Congreso CDMX
            'status': 'Abrogada',  # MODIFICACIÓN: Campo para indicar si tiene "TEXTO VIGENTE" (Vigente/Abrogada)
            'numero_paginas': 0,
            'es_escaneado': False,
            'tiene_tablas': False
        }
        ...
        try:
            ...
            # 5. Detectar status del documento (Vigente/Abrogada)
            if text and len(text.strip()) > 50:
                # MODIFICACIÓN: Detectar si el documento tiene "TEXTO VIGENTE"
                try:
                ...
                result['status'] = 'Abrogada'  # MODIFICACIÓN: Si no hay texto, status es Abrogada
            ...
            # MODIFICACIÓN: Agregados nuevos campos en el resultado de error
            # Devolver resultado mínimo con información del error - Orden reorganizado
            return {
                'titulo': self.clean_filename_to_title(pdf_path.stem),
                'ordenamiento': 'Marco Jurídico',  # MODIFICACIÓN: Campo ordenamiento en error
                'jurisdiccion': 'CIUDAD DE MEXICO',  # NUEVO CAMPO: Jurisdicción en error
                'fuente_oficial': 'Congreso de la Ciudad de México',  # MODIFICACIÓN: Campo fuente_oficial en error
                'status': 'Abrogada',  # MODIFICACIÓN: Campo status en error (default: Abrogada)
                'numero_paginas': 0,
                'es_escaneado': False,
                'tiene_tablas': False,
                'error': str(e)
    ...
    def sanitize_filename(self, titulo: str) -> str:
        ...
        # Limitar longitud a 200 caracteres para evitar problemas
        if len(filename) > 200:
    ...
    def process_all_pdfs(self) -> List[Dict[str, Any]]:
        ...
        for i, pdf_file in enumerate(pdf_files, 1):
            ...
            try:
                ...
                # Buscar URL y fechas correspondientes
                url_data = self.find_matching_url(result['titulo'])
                ...
                ordered_result = {
                    ...
                    'Fecha de Publicación': url_data.get('Fecha de Publicación') if url_data else None,
                    'Fecha de Última Reforma': url_data.get('Fecha de Última Reforma') if url_data else None,
                    'numero_paginas': result['numero_paginas'],
                    'es_escaneado': result['es_escaneado'],
                    'tiene_tablas': result['tiene_tablas'],
                    'url': url_data.get('url') if url_data else None  # URL al final
                }
                ...
                url_data = self.find_matching_url(titulo_temp)
                contenido = self.find_matching_contenido(titulo_temp)

                # MODIFICACIÓN: Agregados nuevos campos en el manejo de errores - Orden reorganizado
                error_result = {
                    'titulo': titulo_temp,
                    'Contenido': contenido if contenido else None,  # Campo Contenido debajo del título
                    'ordenamiento': 'Marco Jurídico',  # MODIFICACIÓN: Campo ordenamiento
                    'jurisdiccion': 'CIUDAD DE MEXICO',  # NUEVO CAMPO: Jurisdicción
                    'fuente_oficial': 'Congreso de la Ciudad de México',  # MODIFICACIÓN: Campo fuente_oficial
                    'status': 'Abrogada',  # MODIFICACIÓN: Campo status (default: Abrogada en error)
                    'Fecha de Publicación': url_data.get('Fecha de Publicación') if url_data else None,
                    'Fecha de Última Reforma': url_data.get('Fecha de Última Reforma') if url_data else None,
                    'numero_paginas': 0,
                    'es_escaneado': False,
                    'tiene_tablas': False,
                    'url': url_data.get('url') if url_data else None,  # URL incluso en caso de error
                    'error': str(e)
                }
...
def main():
    ...
    INPUT_FOLDER = r"C:\Users\julii\Documents\CDMX\marco juridico"
    OUTPUT_FOLDER = r"C:\Users\julii\Documents\Practicas\drive\CDMX\json\marco-juridico"  # Misma carpeta que el script
    ...
    if len(sys.argv) > 1:
        ...
        if pdf_path.exists():
            ...
            # Buscar URL y fechas correspondientes
            url_data = processor.find_matching_url(result['titulo'])
