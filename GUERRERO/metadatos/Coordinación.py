...
class ReglamentoProcessor:
    ...
    def __init__(self, input_folder: str, output_folder: str):
        ...
        self.contenido_json_path = Path(r"C:\Users\julii\Documents\Practicas\drive\GUERRERO\contenido\Coordinación\Coordinación-contenido.json")#contenidos de los PDFs
    ...
    def extract_fecha_publicacion(self, text: str) -> Optional[str]:
        ...
        Busca específicamente el formato del Periódico Oficial:
        Ejemplos:
        - "Ley publicada en el Periódico Oficial, el martes 25 de mayo de 1999"
        - "Código publicado en el Periódico Oficial, el viernes 23 de julio de 1999"
        - "Periódico Oficial del Estado, el lunes 14 de agosto de 2017"
        ...
        patrones = [
            # Patrón 1: ACTAS - "Acta publicada en el Periódico Oficial del Gobierno del Estado No XX Alcance XX, el [día_semana] DD de mes de YYYY"
            r'Acta\s+publicada\s+en\s+el\s+Peri[oó]dico\s+Oficial\s+del\s+Gobierno\s+del\s+Estado\s+No\.?\s*\d+\s*(?:Alcance\s+[IVXLCDM]+)?,?\s*el\s+(?:lunes|martes|mi[eé]rcoles|jueves|viernes|s[aá]bado|domingo)?\s*(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 2: ACTAS - Versión más flexible con saltos de línea
            r'Acta\s+publicada\s+en\s+el\s+Peri[oó]dico\s+Oficial[\s\r\n]+del\s+Gobierno\s+del\s+Estado[\s\r\n]+No\.?\s*\d+[\s\r\n]*(?:Alcance\s+[IVXLCDM]+)?[\s\r\n,]*el\s+(?:lunes|martes|mi[eé]rcoles|jueves|viernes|s[aá]bado|domingo)?\s*(\d{1,2}[\s\r\n]+de\s+\w+[\s\r\n]+de\s+\d{4})',

            # Patrón 3: ACTAS - Versión simplificada
            r'Acta\s+publicada\s+en\s+el\s+Peri[oó]dico\s+Oficial[^,]*,?\s*el\s+(?:lunes|martes|mi[eé]rcoles|jueves|viernes|s[aá]bado|domingo)?\s*(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 4: "[Ley/Código/Reglamento/etc.] publicada/o en el Periódico Oficial, el [día_semana] DD de mes de YYYY"
            r'(?:Ley|C[oó]digo|Reglamento|Decreto|Acuerdo|Norma)\s+publicad[oa]\s+en\s+el\s+Peri[oó]dico\s+Oficial,?\s+el\s+(?:lunes|martes|mi[eé]rcoles|jueves|viernes|s[aá]bado|domingo)\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 5: Sin día de la semana
            r'(?:Ley|C[oó]digo|Reglamento|Decreto|Acuerdo|Norma)\s+publicad[oa]\s+en\s+el\s+Peri[oó]dico\s+Oficial,?\s+el\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 6: Genérico "publicad[oa] en el Periódico Oficial"
            r'publicad[oa]\s+en\s+el\s+Peri[oó]dico\s+Oficial,?\s+el\s+(?:lunes|martes|mi[eé]rcoles|jueves|viernes|s[aá]bado|domingo)\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 4: NUEVO - Sin especificar tipo de documento ni día de la semana
            r'publicad[oa]\s+en\s+el\s+Peri[oó]dico\s+Oficial,?\s+el\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 5: Periódico Oficial del Estado de [Estado], el [día_semana] DD de mes de YYYY
            r'Peri[oó]dico\s+Oficial\s+del\s+Estado(?:\s+de\s+\w+)?,?\s+el\s+(?:lunes|martes|mi[eé]rcoles|jueves|viernes|s[aá]bado|domingo)\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 6: Periódico Oficial sin día de la semana
            r'Peri[oó]dico\s+Oficial\s+del\s+Estado(?:\s+de\s+\w+)?,?\s+el\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 7: Periódico Oficial genérico con saltos de línea
            r'Peri[oó]dico\s+Oficial[\s\r\n]+(?:del\s+Estado[\s\r\n]+)?(?:de\s+\w+[\s\r\n]+)?,?\s+el\s+(?:lunes|martes|mi[eé]rcoles|jueves|viernes|s[aá]bado|domingo)\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 8: "desde su publicación" - MUY FLEXIBLE
            r'desde\s+su\s+publicaci[oó]n[^\d]{0,20}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 9: "desde la publicación" - MUY FLEXIBLE
            r'desde\s+la\s+publicaci[oó]n[^\d]{0,50}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 10: Solo "publicación" seguido de fecha
            r'publicaci[oó]n[^\d]{0,30}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 11: "publicada" o "publicado"
            r'publicad[oa][^\d]{0,20}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 12: "vigente desde"
            r'vigente[^\d]{0,20}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 13: "en vigor"
            r'en\s+vigor[^\d]{0,20}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 14: Simplemente capturar cualquier fecha en formato largo en las primeras líneas
    ...
    def extract_ultima_reforma(self, text: str) -> Optional[str]:
        ...
        Busca específicamente el patrón:
        "ULTIMA REFORMA PUBLICADA EN EL PERIODICO OFICIAL: DD DE MES DE YYYY"
        Ejemplo: "ULTIMA REFORMA PUBLICADA EN EL PERIODICO OFICIAL: 3 DE JUNIO DE 2008"
        ...
        # Patrones ordenados por prioridad (del más específico al más general)
        # NOTA: Todos los patrones capturan SOLO LA FECHA, no el texto completo
        patrones = [
            # Patrón 1: ABROGADO POR... captura solo la fecha al final
            r'ABROGAD[OA]\s+POR\s+.+?PUBLICAD[OA]\s+EN\s+EL\s+PERI[OÓ]DICO\s+OFICIAL\s+DEL\s+GOBIERNO\s+DEL\s+ESTADO\s+No\.?\s*\d+\s*(?:ALCANCE\s+[IVXLCDM]+)?,?\s*EL\s+(?:LUNES|MARTES|MI[EÉ]RCOLES|JUEVES|VIERNES|S[AÁ]BADO|DOMINGO)?\s*(\d{1,2}\s+DE\s+\w+\s+DE\s+\d{4})',

            # Patrón 2: ABROGADO POR... versión flexible - captura solo la fecha
            r'ABROGAD[OA]\s+POR[\s\S]+?PUBLICAD[OA]\s+EN\s+EL\s+PERI[OÓ]DICO\s+OFICIAL[\s\S]+?(\d{1,2}\s+DE\s+\w+\s+DE[L]?\s+\d{4})',

            # Patrón 3: POR TERMINADA SU VIGENCIA - captura solo la fecha
            r'POR\s+T[EÉ]RMINADA\s+SU\s+VIGENCIA\s+EL\s+(\d{1,2}\s+DE\s+\w+\s+DE[L]?\s+\d{4})',

            # Patrón 4: POR TERMINADA SU VIGENCIA - versión flexible - captura solo la fecha
            r'POR\s+T[EÉ]RMINADA\s+SU\s+VIGENCIA[\s\S]{0,30}(\d{1,2}\s+DE\s+\w+\s+DE[L]?\s+\d{4})',

            # Patrón 5: DEROGADO POR... - captura solo la fecha
            r'DEROGAD[OA]\s+POR\s+.+?PUBLICAD[OA]\s+EN\s+EL\s+PERI[OÓ]DICO\s+OFICIAL[\s\S]+?(\d{1,2}\s+DE\s+\w+\s+DE[L]?\s+\d{4})',

            # Patrón 6: "ULTIMA REFORMA PUBLICADA EN EL PERIODICO OFICIAL: DD DE MES DE YYYY"
            r'[UÚ]LTIMA\s+REFORMA\s+PUBLICADA\s+EN\s+EL\s+PERI[OÓ]DICO\s+OFICIAL\s*:?\s*(\d{1,2}\s+DE\s+\w+\s+DE\s+\d{4})',

            # Patrón 7: Versión en mayúsculas/minúsculas mezcladas
            r'[UÚ]ltima\s+[Rr]eforma\s+[Pp]ublicada\s+en\s+el\s+Peri[oó]dico\s+Oficial\s*:?\s*(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 8: Sin "en el"
            r'[UÚ]LTIMA\s+REFORMA\s+PUBLICADA\s+PERI[OÓ]DICO\s+OFICIAL\s*:?\s*(\d{1,2}\s+DE\s+\w+\s+DE\s+\d{4})',

            # Patrón 9: Formato más corto
            r'[UÚ]ltima\s+[Rr]eforma\s+[Pp]ublicada\s*:?\s*(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 10: "Última reforma" - MUY FLEXIBLE - formato largo
            r'[UÚ]ltima\s+[Rr]eforma[^\d]{0,50}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 6: "Última actualización" - formato DD/MES/YYYY
            r'[UÚ]ltima\s+[Aa]ctualizaci[oó]n[^\d]{0,30}(\d{1,2}/\w+/\d{4})',

            # Patrón 7: "Última actualización" - formato DD/MM/YYYY
            r'[UÚ]ltima\s+[Aa]ctualizaci[oó]n[^\d]{0,30}(\d{1,2}/\d{1,2}/\d{4})',

            # Patrón 8: "Última reforma" - formato corto DD-MM-YYYY
            r'[UÚ]ltima\s+[Rr]eforma[^\d]{0,30}(\d{1,2}-\d{1,2}-\d{4})',

            # Patrón 9: "Última modificación"
            r'[UÚ]ltima\s+[Mm]odificaci[oó]n[^\d]{0,30}(\d{1,2}[-/]\d{1,2}[-/]\d{4})',
            r'[UÚ]ltima\s+[Mm]odificaci[oó]n[^\d]{0,30}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 10: Solo "reforma" seguido de fecha
            r'[Rr]eforma[^\d]{0,50}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',
            r'[Rr]eforma[^\d]{0,30}(\d{1,2}[-/]\d{1,2}[-/]\d{4})',

            # Patrón 11: Solo "actualización" seguido de fecha
            r'[Aa]ctualizaci[oó]n[^\d]{0,30}(\d{1,2}[-/]\d{1,2}[-/]\d{4})',
            r'[Aa]ctualizaci[oó]n[^\d]{0,30}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 12: "modificación" genérica
            r'[Mm]odificaci[oó]n[^\d]{0,30}(\d{1,2}[-/]\d{1,2}[-/]\d{4})',
            r'[Mm]odificaci[oó]n[^\d]{0,30}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 13: Captura genérica - cualquier fecha en formato DD/MES/YYYY (como 5/ABRIL/2021)
        ...
        for i, patron in enumerate(patrones, 1):
            ...
            if match:
                ...
                logger.info(f"Última reforma extraída con patrón #{i}: {resultado}")
                return resultado
    ...
    def detect_status(self, text: str) -> str:
        """
        Detecta el status del documento (VIGENTE o ABROGADA)

        Busca en la primera hoja indicadores de que el documento está vigente o abrogado.
        Si encuentra "ABROGADA/ABROGADO/DEROGADA/DEROGADO" -> retorna "ABROGADA"
        Si encuentra "TEXTO VIGENTE/TEXTO ORIGINAL/EN VIGOR" o no encuentra nada -> retorna "VIGENTE"
        (Por defecto es VIGENTE ya que los PDFs vienen del diario oficial)
        ...
        """
        if not text:
            return "VIGENTE"  # Por defecto vigente

        # Buscar en los primeros 5000 caracteres (aproximadamente primera hoja)
        text_inicio = text[:5000] if len(text) > 5000 else text

        # Primero buscar si está ABROGADA/DEROGADA (tiene prioridad)
        patrones_abrogada = [
            r'(?i)\babrogad[oa]\b',
            r'(?i)\bderogad[oa]\b',
            r'(?i)\bsin\s+vigencia\b',
            r'(?i)\bno\s+vigente\b',
            r'(?i)\bfuera\s+de\s+vigor\b',
            r'(?i)\bpor\s+t[eé]rminada\s+su\s+vigencia\b'
        ]

        for patron in patrones_abrogada:
            if re.search(patron, text_inicio):
                logger.info(f"Status 'ABROGADA' encontrado con patrón: {patron}")
                return "ABROGADA"

        # Buscar indicadores de vigencia (opcional, ya que por defecto es VIGENTE)
        patrones_vigente = [
            r'(?i)texto\s+vigente',
            r'(?i)texto\s+original',
            r'(?i)texto\s+en\s+vigor',
            r'(?i)\ben\s+vigor\b',
            r'(?i)\bvigente\b'
        ]

        for patron in patrones_vigente:
            if re.search(patron, text_inicio):
                logger.info(f"Status 'VIGENTE' encontrado con patrón: {patron}")
                return "VIGENTE"

        # Por defecto es VIGENTE (los PDFs vienen del diario oficial)
        logger.info("No se encontró indicador de status, asumiendo VIGENTE")
        return "VIGENTE"
    ...
    def process_single_pdf(self, pdf_path: Path) -> Dict[str, Any]:
        ...
        result = {
            ...
            'ordenamiento': 'ACTA',  # Tipo de ordenamiento
            'jurisdiccion': 'GUERRERO',  # Jurisdicción
            'fuente_oficial': 'DIARIO OFICIAL DE LA FEDERACION',  # Fuente oficial
            'Fecha de Publicación': None,  # Se extraerá del PDF
            'ULTIMA REFORMA PUBLICADA': None,  # Se extraerá del PDF
            'status': 'VIGENTE',  # Se extraerá del PDF (VIGENTE o ABROGADA)
            'es_escaneado': False,
            'tiene_tablas': False
        }
        ...
        try:
            ...
            # 5. Extraer fecha de publicación del PDF
            if text and len(text.strip()) > 50:
                try:
                    fecha_pub = self.extract_fecha_publicacion(text)
                    result['Fecha de Publicación'] = fecha_pub
                except Exception as e:
                    logger.warning(f"Error extrayendo fecha de publicación: {e}")
                    result['Fecha de Publicación'] = None
            else:
                result['Fecha de Publicación'] = None

            # 6. Extraer última reforma publicada del PDF
            if text and len(text.strip()) > 50:
                try:
                    ultima_reforma = self.extract_ultima_reforma(text)
                    result['ULTIMA REFORMA PUBLICADA'] = ultima_reforma
                except Exception as e:
                    logger.warning(f"Error extrayendo última reforma: {e}")
                    result['ULTIMA REFORMA PUBLICADA'] = None
            else:
                result['ULTIMA REFORMA PUBLICADA'] = None

            # 7. Detectar status del documento (VIGENTE o ABROGADA)
            if text and len(text.strip()) > 50:
                try:
                    result['status'] = self.detect_status(text)
                    logger.info(f"Status: {result['status']}")
                except Exception as e:
                    logger.warning(f"Error detectando status: {e}")
                    result['status'] = 'VIGENTE'  # Por defecto vigente
            else:
                result['status'] = 'VIGENTE'  # Por defecto vigente
        ...
        for i, pdf_file in enumerate(pdf_files, 1):
            ...
            try:
                ...
                ordered_result = {
                    ...
                    'Fecha de Publicación': result['Fecha de Publicación'],  # Extraída del PDF
                    'ULTIMA REFORMA PUBLICADA': result['ULTIMA REFORMA PUBLICADA'],  # Extraída del PDF
                    'status': result['status'],  # VIGENTE o ABROGADA
...
def main():
    ...
    INPUT_FOLDER = r"C:\Users\julii\Documents\GUERRERO DOF\Coordinación" # Carpeta con los PDFs
    OUTPUT_FOLDER = r"C:\Users\julii\Documents\Practicas\drive\GUERRERO\json\Coordinación"  # Misma carpeta que el script
