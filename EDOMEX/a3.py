...
class ReglamentoProcessor:
    ...
    def __init__(self, input_folder: str, output_folder: str):
        ...
        # Ruta del JSON con URLs
        self.url_json_path = Path(r"C:\Users\julii\Documents\Practicas\drive\EDOMEX\json\A1 METADATOS\leyes_federales.json")# metadatos
        self.url_data = None

        # Ruta del JSON con contenidos
        self.contenido_json_path = Path(r"C:\Users\julii\Documents\Practicas\drive\EDOMEX\json\B1 CONTENIDO\LF-contenido.json")#contenidos de los PDFs
    ...
    def load_url_data(self):
        """Carga el JSON con las URLs de los reglamentos"""
        try:
            if self.url_json_path.exists():
                with open(self.url_json_path, 'r', encoding='utf-8') as f:
                    self.url_data = json.load(f)
                logger.info(f"Cargadas {len(self.url_data)} URLs desde {self.url_json_path.name}")
                return True
            else:
                logger.warning(f"No se encontró el archivo de URLs: {self.url_json_path}")
                return False
        except Exception as e:
            logger.error(f"Error cargando JSON de URLs: {e}")
            return False
    ...
    def find_matching_url(self, titulo: str) -> Optional[str]:
        """
        Busca la URL correspondiente al título del documento
        Retorna solo la URL si encuentra match, None si no
        """
        if not self.url_data or not titulo:
            return None
        ...
        for item in self.url_data:
            # Obtener el título del JSON de URLs - Buscar con "TITULO" (mayúsculas o minúsculas)
            ...
            if titulo_normalizado == titulo_json_normalizado:
                logger.info(f"Match exacto encontrado: {titulo[:50]}...")
                return item.get('URL') or item.get('URL_PDF')
            ...
            if ratio > best_ratio and ratio > 0.92:
                ...
                best_url = item.get('URL') or item.get('URL_PDF')

        if best_url:
            logger.info(f"Mejor match encontrado (similaridad {best_ratio:.2%}): {titulo[:50]}... -> {best_match[:50]}...")
            return best_url

        logger.warning(f"No se encontró URL para: {titulo[:50]}...")
        return None
    ...
    def extract_fecha_publicacion(self, text: str) -> Optional[str]:
        ...
        Busca patrones como: "Nueva Ley publicada en el Diario Oficial de la Federación el 1 de junio de 2006"
        """
        if not text:
            return None

        # Buscar en los primeros 5000 caracteres (primera hoja)
        text_inicio = text[:5000] if len(text) > 5000 else text

        # Patrones para detectar la fecha de publicación (para LEYES)
        patrones = [
            r'Nueva\s+Ley\s+publicada\s+en\s+el\s+Diario\s+Oficial\s+de\s+la\s+Federaci[oó]n\s+el\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',
            r'publicada\s+en\s+el\s+Diario\s+Oficial\s+de\s+la\s+Federaci[oó]n\s+el\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',
            r'publicada\s+en\s+el\s+DOF\s+el\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',
            r'Diario\s+Oficial\s+de\s+la\s+Federaci[oó]n\s+el\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',
            r'DOF\s+el\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',
            r'publicaci[oó]n[:\s]+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',
        ]
        ...
        for patron in patrones:
            ...
            if match:
                ...
                logger.info(f"Fecha de publicación extraída: {fecha}")
                return fecha
    ...
    def extract_ultima_reforma(self, text: str) -> Optional[str]:
        """
        Extrae la última reforma publicada del PDF
        Busca patrones como: "Última reforma publicada DOF 20-05-2021"
        """
        if not text:
            return None
        ...
        # Patrones para detectar la última reforma
        patrones = [
            r'[UÚ]ltima\s+reforma\s+publicada\s+DOF\s+(\d{1,2}-\d{1,2}-\d{4})',
            r'[UÚ]ltima\s+reforma\s+publicada\s+en\s+el\s+DOF\s+(\d{1,2}-\d{1,2}-\d{4})',
            r'[UÚ]ltima\s+reforma\s+DOF\s+(\d{1,2}-\d{1,2}-\d{4})',
            r'reforma\s+publicada\s+DOF\s+(\d{1,2}-\d{1,2}-\d{4})',
        ]
        ...
        for patron in patrones:
            ...
            if match:
                fecha = match.group(1).strip()
                logger.info(f"Última reforma publicada extraída: {fecha}")
                return fecha

        logger.warning("No se encontró última reforma publicada en el PDF")
        return None
    ...
    def process_single_pdf(self, pdf_path: Path) -> Dict[str, Any]:
        ...
        result = {
            ...
            'ordenamiento': 'LEYES FEDERALES',  # Tipo de ordenamiento
            'jurisdiccion': 'ESTADO DE MÉXICO',  # Jurisdicción
            'fuente_oficial': 'DIARIO OFICIAL DE LA FEDERACION',  # Fuente oficial
            'status': 'Abrogada',  # Status inicial, se actualizará al detectar "TEXTO VIGENTE"
            'Fecha de Publicación': None,  # Se extraerá del PDF
            'Última Reforma Publicada': None,  # Se extraerá del PDF
            'es_escaneado': False,
            'tiene_tablas': False
        }
        ...
        try:
            ...
            # 5. Detectar status del documento (Vigente/Abrogada) y extraer fechas
            if text and len(text.strip()) > 50:
                # Detectar si el documento tiene "TEXTO VIGENTE"
                try:
                    tiene_texto_vigente = self.detect_texto_vigente(text)
                    result['status'] = 'Vigente' if tiene_texto_vigente else 'Abrogada'
                    logger.info(f"Status del documento: {result['status']}")
                except Exception as e:
                    logger.warning(f"Error detectando status: {e}")
                    result['status'] = 'Abrogada'

                # Extraer fecha de publicación del PDF
                try:
                ...
                # Extraer última reforma publicada del PDF
                try:
                    ultima_reforma = self.extract_ultima_reforma(text)
                    result['Última Reforma Publicada'] = ultima_reforma
                except Exception as e:
                    logger.warning(f"Error extrayendo última reforma: {e}")
                    result['Última Reforma Publicada'] = None
            else:
                result['status'] = 'Abrogada'  # Si no hay texto, status es Abrogada
                result['Fecha de Publicación'] = None
                result['Última Reforma Publicada'] = None
            ...
            return {
                ...
                'ordenamiento': 'LEYES FEDERALES',
                'jurisdiccion': 'ESTADO DE MÉXICO',
                'fuente_oficial': 'DIARIO OFICIAL DE LA FEDERACION',
                'status': 'Abrogada',  # Default en caso de error
                'Fecha de Publicación': None,
                'Última Reforma Publicada': None,
                'es_escaneado': False,
                'tiene_tablas': False,
                'error': str(e)
            }
    ...
    def process_all_pdfs(self) -> List[Dict[str, Any]]:
        ...
        logger.info("\n" + "="*60)
        logger.info("Cargando archivo de URLs...")
        self.load_url_data()
        ...
        for i, pdf_file in enumerate(pdf_files, 1):
            ...
            try:
                ...
                url = self.find_matching_url(result['titulo'])

                # Buscar Contenido correspondiente
                contenido = self.find_matching_contenido(result['titulo'])

                # Reorganizar el diccionario con el orden deseado incluyendo 'url' y 'Contenido'
                ordered_result = {
                    ...
                    'Última Reforma Publicada': result['Última Reforma Publicada'],  # Extraída del PDF
                    'es_escaneado': result['es_escaneado'],
                    'tiene_tablas': result['tiene_tablas'],
                    'url': url if url else None  # URL al final
                }
                ...
                # Intentar buscar URL y Contenido incluso en caso de error
                titulo_temp = self.clean_filename_to_title(pdf_file.stem)
                url = self.find_matching_url(titulo_temp)
                ...
                error_result = {
                    ...
                    'es_escaneado': False,
                    'tiene_tablas': False,
                    'url': url if url else None,  # URL incluso en caso de error
                    'error': str(e)
                }
        ...
        total_con_url = sum(1 for d in documentos if d.get('url') is not None)
        total_sin_url = len(documentos) - total_con_url
        ...
        logger.info(f"✓ URLs encontradas: {total_con_url}/{len(documentos)}")
        if total_sin_url > 0:
            logger.info(f"⚠ Sin URL: {total_sin_url} documentos")
...
def main():
    ...
    INPUT_FOLDER = r"C:\Users\julii\Documents\EDOMEX\LEYES FEDERALES" # Carpeta con los PDFs
    OUTPUT_FOLDER = r"C:\Users\julii\Documents\Practicas\drive\EDOMEX\json\LEYES FEDERALES"  # Misma carpeta que el script
    ...
    if len(sys.argv) > 1:
        ...
        if pdf_path.exists():
            ...
            # Cargar URLs y Contenidos si están disponibles
            processor.load_url_data()
            processor.load_contenido_data()

            result = processor.process_single_pdf(pdf_path)

            # Buscar URL correspondiente
            url = processor.find_matching_url(result['titulo'])
