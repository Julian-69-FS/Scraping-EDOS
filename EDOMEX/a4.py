...
class ReglamentoProcessor:
    ...
    def __init__(self, input_folder: str, output_folder: str):
        ...
        self.url_json_path = Path(r"C:\Users\julii\Documents\Practicas\drive\EDOMEX\json\A1 METADATOS\leyes_y_codigos.json")# metadatos
        self.url_data = None

        # Ruta del JSON con contenidos
        self.contenido_json_path = Path(r"C:\Users\julii\Documents\Practicas\drive\EDOMEX\json\B1 CONTENIDO\LC-contenido.json")#contenidos de los PDFs
    ...
    def extract_fecha_publicacion(self, text: str) -> Optional[str]:
        ...
        - "desde su publicación 1 de julio de 2002"
        - "desde la publicación de la Constitución, el 24 de febrero de 1995"
        - "desde su publicación, 23 de octubre de 1998"
        """
        if not text:
            return None
        ...
        # Formato de fecha: DD de MES de YYYY o DD/MM/YYYY o DD-MM-YYYY
        patrones = [
            # Patrón 1: "desde su publicación" - MUY FLEXIBLE
            r'desde\s+su\s+publicaci[oó]n[^\d]{0,20}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 2: "desde la publicación" - MUY FLEXIBLE
            r'desde\s+la\s+publicaci[oó]n[^\d]{0,50}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 3: Solo "publicación" seguido de fecha
            r'publicaci[oó]n[^\d]{0,30}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 4: "publicada" o "publicado"
            r'publicad[oa][^\d]{0,20}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 5: "vigente desde"
            r'vigente[^\d]{0,20}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 6: "en vigor"
            r'en\s+vigor[^\d]{0,20}(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',

            # Patrón 7: Simplemente capturar cualquier fecha en formato largo en las primeras líneas
        ...
        for patron in patrones:
            ...
            if match:
                ...
                logger.info(f"Fecha de publicación extraída con patrón: {fecha}")
                return fecha
    ...
    def process_single_pdf(self, pdf_path: Path) -> Dict[str, Any]:
        ...
        result = {
            ...
            'ordenamiento': 'LEYES Y CÓDIGOS',  # Tipo de ordenamiento
...
def main():
    ...
    INPUT_FOLDER = r"C:\Users\julii\Documents\EDOMEX\LEYES Y CÓDIGOS" # Carpeta con los PDFs
    OUTPUT_FOLDER = r"C:\Users\julii\Documents\Practicas\drive\EDOMEX\json\LEYES Y CÓDIGOS"  # Misma carpeta que el script
