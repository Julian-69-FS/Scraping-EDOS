...
def corregir_saltos_linea(texto: str) -> str:
    """Une líneas hasta formar bloques completos de texto

    FILOSOFÍA AGRESIVA:
    - UNIR solo cuando es OBVIO que continúa (conectores, sin puntuación + minúscula)
    - CREAR SALTO en la mayoría de casos:
      * Termina con cualquier puntuación (. ; : ,)
      * Siguiente línea empieza con mayúscula
      * Línea vacía
      * Nueva estructura (Artículo, Fracción, etc.)
    - Prioridad: MARCAR MÁS SALTOS, ser conservador al unir
    """
    if not texto:
        return texto
    ...
    for i, linea in enumerate(lineas):
        ...
        # Detectar INICIO de estructura nueva (Artículo, Capítulo, Fracción, etc.)
        es_inicio_estructura = re.match(
            r'^(Artículo\s+\d+|ARTÍCULO\s+\d+|Capítulo\s+[IVX]+|CAPÍTULO\s+[IVX]+|'
            r'TÍTULO|Título|SECCIÓN|Sección|'
            r'[IVX]+\.|[A-Z]\)|a\)|[0-9]+\.|[0-9]+\)|'
            r'Fracción|DISPOSICIONES|TRANSITORIOS|Al margen|PRIMERO|SEGUNDO|TERCERO)',
            linea_limpia
        )

        if es_inicio_estructura and bloque_actual:
            # Guardar bloque anterior y empezar nuevo
            bloques.append(' '.join(bloque_actual))
            bloque_actual = []

        # Palabra cortada con guión: unir sin guión
        if bloque_actual and bloque_actual[-1].endswith('-'):
            bloque_actual[-1] = bloque_actual[-1][:-1] + linea_limpia
            continue

        # Agregar línea al bloque actual
        bloque_actual.append(linea_limpia)

        # Determinar si DEBE TERMINAR el bloque aquí
        debe_terminar_bloque = False

        # CASO 1: Termina con puntuación fuerte (. ; :)
        if re.search(r'[.;:]\s*$', linea_limpia):
            debe_terminar_bloque = True

        # CASO 2: Termina con coma Y la siguiente línea empieza con MAYÚSCULA
        elif linea_limpia.endswith(','):
            # Ver la siguiente línea
            if i + 1 < len(lineas):
                siguiente = lineas[i + 1].strip()
                if siguiente and re.match(r'^[A-ZÁÉÍÓÚÑ]', siguiente):
                    # Siguiente empieza con mayúscula = fin de bloque
                    debe_terminar_bloque = True

        # CASO 3: NO termina con puntuación pero siguiente empieza con MAYÚSCULA
        # (probable fin de párrafo)
        elif not re.search(r'[.,;:]\s*$', linea_limpia):
            if i + 1 < len(lineas):
                siguiente = lineas[i + 1].strip()
                if siguiente:
                    # Si siguiente es estructura, no evaluar aquí (se maneja arriba)
                    es_sig_estructura = re.match(
                        r'^(Artículo|ARTÍCULO|Capítulo|CAPÍTULO|TÍTULO|Título|'
                        r'[IVX]+\.|[A-Z]\)|a\)|[0-9]+\.|[0-9]+\))',
                        siguiente
                    )
                    if not es_sig_estructura and re.match(r'^[A-ZÁÉÍÓÚÑ]', siguiente):
                        # Siguiente empieza con mayúscula y NO es estructura
                        # Solo terminar si NO termina con conector obvio
                        no_termina_conector = not re.search(
                            r'\b(y|e|o|u|de|del|al|el|la|los|las|un|una|en|con|sin)\s*$',
                            linea_limpia,
                            re.IGNORECASE
                        )
                        if no_termina_conector:
                            debe_terminar_bloque = True

        if debe_terminar_bloque:
            bloques.append(' '.join(bloque_actual))
            bloque_actual = []
    ...
    # Unir bloques con saltos de línea
    texto_final = '\n'.join(bloques)
...
def eliminar_encabezados_pies_contextual(paginas_texto: List[str], encabezados: List[str], pies: List[str]) -> List[str]:
    ...
    # Patrones GENÉRICOS y agresivos de encabezados/pies a eliminar
    patrones_eliminar = [
        # Patrones de Diario Oficial
        r'DIARIO OFICIAL',
        r'Diario Oficial',
        r'(Primera|Segunda|Tercera|Cuarta)\s+(Sección|SECCIÓN)',
        ...
        r'^Página\s+\d+',
        r'^Page\s+\d+',
        r'^\d+\s+de\s+\d+\s*$',  # "1 de 21", "2 de 21"
        r'^\d+\s+\(\w+\s+(Sección|SECCIÓN)\)',

        # Patrones genéricos de instituciones (más flexibles)
        r'CÁMARA DE DIPUTADOS',
        r'CONGRESO DE LA UNIÓN',
        r'H\.\s+CONGRESO',
        r'^Secretaría\s+(General|de\s+\w+)',  # Cualquier Secretaría
        r'^Secretaría\s+de\s+Servicios',

        # Patrones de reglamentos/leyes (genéricos)
        ...
        # Patrones de títulos largos en mayúsculas (típicos de encabezados)
        r'^[A-ZÁÉÍÓÚÑ\s]{40,}$',  # Líneas de solo mayúsculas muy largas
    ]
    ...
    for pagina in paginas_texto:
        ...
        for i, linea in enumerate(lineas):
            ...
            # Verificar patrones agresivos en las primeras 8 líneas (más líneas para encabezados complejos)
            if i < 8:
            ...
            # Verificar patrones agresivos en las últimas 8 líneas
            if i >= len(lineas) - 8:
                for patron in patrones_eliminar:
                    if re.search(patron, linea_stripped, re.IGNORECASE):
                        es_pie = True
...
def main():
    ...
    ruta_carpeta = r"C:\Users\julii\Documents\EDOMEX\REGLAMENTOS DE LEYES FEDERALES"

    # Nombre del archivo JSON de salida
    archivo_salida = r"C:\Users\julii\Documents\Practicas\drive\EDOMEX\json\B1 CONTENIDO\RLF-contenido.json"
